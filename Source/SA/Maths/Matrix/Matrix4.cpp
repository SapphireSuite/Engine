		...
		*/
		...
		const __m256 row01 = _mm256_add_ps(
			_mm256_add_ps(
				...
				_mm256_add_ps(r1rp3_r0rp3, r1rp4_r0rp4)),
			_mm256_add_ps(r1rp5_r0rp5, r1rp6_r0rp6)
		);
//}
		...
		*/
		...
		const __m256 row23 = _mm256_add_ps(
			_mm256_add_ps(
				...
				_mm256_add_ps(r3rp3_r2rp3, r3rp4_r2rp4)),
			_mm256_add_ps(r3rp5_r2rp5, r3rp6_r2rp6)
		);
//}
	...
	RMat4f& RMat4f::operator*=(float _scale) noexcept
	{
		...
		// Mult row 0 and 1.
		const __m256 res01 = _mm256_mul_ps(_mm256_load_ps(&data[0]), sPack);
		_mm256_store_ps(&data[0], res01);

		// Mult row 2 and 3.
		const __m256 res23 = _mm256_mul_ps(_mm256_load_ps(&data[8]), sPack);
		_mm256_store_ps(&data[8], res23);
	...
	RMat4f& RMat4f::operator/=(float _scale) noexcept
	{
		...
		// Div row 0 and 1.
		const __m256 res01 = _mm256_div_ps(_mm256_load_ps(&data[0]), sPack);
		_mm256_store_ps(&data[0], res01);

		// Div row 2 and 3.
		const __m256 res23 = _mm256_div_ps(_mm256_load_ps(&data[8]), sPack);
		_mm256_store_ps(&data[8], res23);
	...
	RMat4f& RMat4f::operator+=(const RMat4f& _rhs) noexcept
	{
		...
		// Add row 0 and 1.
		const __m256 res01 = _mm256_add_ps(_mm256_load_ps(&ldata[0]), _mm256_load_ps(&rdata[0]));
		_mm256_store_ps(&ldata[0], res01);

		// Add row 2 and 3.
		const __m256 res23 = _mm256_add_ps(_mm256_load_ps(&ldata[8]), _mm256_load_ps(&rdata[8]));
		_mm256_store_ps(&ldata[8], res23);
	...
	RMat4f& RMat4f::operator-=(const RMat4f& _rhs) noexcept
	{
		...
		// Sub row 0 and 1.
		const __m256 res01 = _mm256_sub_ps(_mm256_load_ps(&ldata[0]), _mm256_load_ps(&rdata[0]));
		_mm256_store_ps(&ldata[0], res01);

		// Sub row 2 and 3.
		const __m256 res23 = _mm256_sub_ps(_mm256_load_ps(&ldata[8]), _mm256_load_ps(&rdata[8]));
		_mm256_store_ps(&ldata[8], res23);
...
//{ Row 0
		...
		*/
		...
		const __m256d row0 = _mm256_mul_pd(invDetP,
			_mm256_add_pd(
				_mm256_add_pd(
					_mm256_add_pd(r0rp1, r0rp2),
					_mm256_add_pd(r0rp3, r0rp4)
				),
				_mm256_add_pd(r0rp5, r0rp6)
			)
		);
...
//{ Row 1
		...
		*/
		...
		const __m256d row1 = _mm256_mul_pd(invDetP,
			_mm256_add_pd(
				_mm256_add_pd(
					_mm256_add_pd(r1rp1, r1rp2),
					_mm256_add_pd(r1rp3, r1rp4)
				),
				_mm256_add_pd(r1rp5, r1rp6)
			)
		);
...
//{ Row 2
		...
		*/
		...
		const __m256d row2 = _mm256_mul_pd(invDetP,
			_mm256_add_pd(
				_mm256_add_pd(
					_mm256_add_pd(r2rp1, r2rp2),
					_mm256_add_pd(r2rp3, r2rp4)
				),
				_mm256_add_pd(r2rp5, r2rp6)
			)
		);
...
//{ Row 3
		...
		*/
		...
		const __m256d row3 = _mm256_mul_pd(invDetP,
			_mm256_add_pd(
				_mm256_add_pd(
					_mm256_add_pd(r3rp1, r3rp2),
					_mm256_add_pd(r3rp3, r3rp4)
				),
				_mm256_add_pd(r3rp5, r3rp6)
			)
		);
...
//}
	...
	RMat4d& RMat4d::operator*=(double _scale) noexcept
	{
		...
		// Mult row 0.
		const __m256d row0 = _mm256_mul_pd(_mm256_load_pd(&data[0]), sPack);
		_mm256_store_pd(&data[0], row0);

		// Mult row 1.
		const __m256d row1 = _mm256_mul_pd(_mm256_load_pd(&data[4]), sPack);
		_mm256_store_pd(&data[4], row1);

		// Mult row 2.
		const __m256d row2 = _mm256_mul_pd(_mm256_load_pd(&data[8]), sPack);
		_mm256_store_pd(&data[8], row2);

		// Mult row 3.
		const __m256d row3 = _mm256_mul_pd(_mm256_load_pd(&data[12]), sPack);
		_mm256_store_pd(&data[12], row3);
	...
	RMat4d& RMat4d::operator/=(double _scale) noexcept
	{
		...
		// Div row 0.
		const __m256d row0 = _mm256_div_pd(_mm256_load_pd(&data[0]), sPack);
		_mm256_store_pd(&data[0], row0);

		// Div row 1.
		const __m256d row1 = _mm256_div_pd(_mm256_load_pd(&data[4]), sPack);
		_mm256_store_pd(&data[4], row1);

		// Div row 2.
		const __m256d row2 = _mm256_div_pd(_mm256_load_pd(&data[8]), sPack);
		_mm256_store_pd(&data[8], row2);

		// Div row 3.
		const __m256d row3 = _mm256_div_pd(_mm256_load_pd(&data[12]), sPack);
		_mm256_store_pd(&data[12], row3);
	...
	RMat4d& RMat4d::operator+=(const RMat4d& _rhs) noexcept
	{
		...
		// Add row 0.
		const __m256d row0 = _mm256_add_pd(_mm256_load_pd(&ldata[0]), _mm256_load_pd(&rdata[0]));
		_mm256_store_pd(&ldata[0], row0);

		// Add row 1.
		const __m256d row1 = _mm256_add_pd(_mm256_load_pd(&ldata[4]), _mm256_load_pd(&rdata[4]));
		_mm256_store_pd(&ldata[4], row1);

		// Add row 2.
		const __m256d row2 = _mm256_add_pd(_mm256_load_pd(&ldata[8]), _mm256_load_pd(&rdata[8]));
		_mm256_store_pd(&ldata[8], row2);

		// Add row 3.
		const __m256d row3 = _mm256_add_pd(_mm256_load_pd(&ldata[12]), _mm256_load_pd(&rdata[12]));
		_mm256_store_pd(&ldata[12], row3);
	...
	RMat4d& RMat4d::operator-=(const RMat4d& _rhs) noexcept
	{
		...
		// Sub row 0.
		const __m256d row0 = _mm256_sub_pd(_mm256_load_pd(&ldata[0]), _mm256_load_pd(&rdata[0]));
		_mm256_store_pd(&ldata[0], row0);

		// Sub row 1.
		const __m256d row1 = _mm256_sub_pd(_mm256_load_pd(&ldata[4]), _mm256_load_pd(&rdata[4]));
		_mm256_store_pd(&ldata[4], row1);

		// Sub row 2.
		const __m256d row2 = _mm256_sub_pd(_mm256_load_pd(&ldata[8]), _mm256_load_pd(&rdata[8]));
		_mm256_store_pd(&ldata[8], row2);

		// Sub row 3.
		const __m256d row3 = _mm256_sub_pd(_mm256_load_pd(&ldata[12]), _mm256_load_pd(&rdata[12]));
		_mm256_store_pd(&ldata[12], row3);
