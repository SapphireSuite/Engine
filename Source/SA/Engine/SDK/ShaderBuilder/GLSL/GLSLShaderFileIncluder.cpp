...
namespace Sa::GLSL
{
	...
	{
		return new shaderc_include_result{ nullptr, 0u, message.c_str(), message.size(), nullptr };
	}
	...
	std::string ShaderFileIncluder::GetFullPath(const std::string& _requestedSrc, const std::string& _requestingSrc, shaderc_include_type _type)
	{
		switch (_type)
		{
			...
			{
				const size_t dirIndex = _requestingSrc.find_last_of("/");
			...
			{
				static const std::string shaderLibPath = "Resources/Shaders/Lib";

				return shaderLibPath + "/" + _requestedSrc;
			}
			default:
			{
				SA_LOG(L"Shaderc include type [" << _type << L"] not supported yet!", Error, SA/Engine/SDK/Asset);
				return "";
			}
		}
	}
		...
		size_t _includeDepth)
	{
		...
		if (fullPath.empty())
			return MakeError(("Can't find included file {"_L << _requestedSrc << "} from {" << _requestingSrc << "}").str);
		...
		if(!shaderc_util::ReadFile(fullPath, &content))
			return MakeError(("Can't read included file {"_L << _requestedSrc << "} from {" << _requestingSrc << "}").str);
