...
namespace Sa::Vk
{
	void ImageBuffer::CreateImage(const Device& _device, const VkImageCreateInfo& _vkInfos)
	{
		...
		const uint32_t memoryTypeIndex = ABuffer::FindMemoryType(_device, memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	...
	void ImageBuffer::CopyBufferToImage(ResourceInitializer& _init, const ImageBuffer::CopyBufferImageInfos& _infos)
	{
		...
		uint32_t layerCount = API_GetLayerNum(_infos.imageType);
		uint32_t channelNum = API_GetChannelNum(_infos.format);

		std::vector<VkBufferImageCopy>& bufferImageCopies = _init.resHolder.Make<std::vector<VkBufferImageCopy>>();

		for (uint32_t currMipLevel = 0u; currMipLevel < _infos.mipLevels; ++currMipLevel)
		{
		...
		vkCmdCopyBufferToImage(_init.cmd, _infos.buffer, mImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, (uint32_t)bufferImageCopies.size(), bufferImageCopies.data());
	}
