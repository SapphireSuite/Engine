...
namespace SA::VK
{
	void Cubemap::Create(const Device& _device, ResourceInitializer& _init, const RawCubemap& _raw)
	{
		...
		ObjectBuffer& irrStagingBuffer = CGPUBuffer::CreateStaging(_device, _init, _raw.irradiancemapData.data(), _raw.GetMapSize());
	...
	VkWriteDescriptorSet Cubemap::CreateWriteDescriptorSet(VkDescriptorSet _descriptorSet, uint32_t _binding, uint32_t _arrayElem) noexcept
	{
