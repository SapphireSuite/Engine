...
#include <Render/Vulkan/Device/VkCommandPool.hpp>
...
namespace Sa::Vk
{
	...
	std::vector<CommandBuffer> CommandPool::AllocateMultiple(const Device& _device, uint32 _num, VkCommandBufferLevel _level)
	{
		...
		SA_VK_ASSERT(vkAllocateCommandBuffers(_device, &commandBufferAllocInfo, vkCommandBuffers.data()), L"Failed to allocate command buffers!");
